...
class ShareFileCommand extends Command {
    ...
    async execute(){
        ...
        if (userIsAssociatedWithFile) {
            this.fileManager.updateUserFilePermissions(this.accountUserToShareFileWith, this.selectedFile.ipfsCID, permissionsArray);
        ...
        this.fileManager.fileShare(this.accountUserToShareFileWith, this.selectedFile.ipfsCID, encryptedSymmetricKeyShared.toString('base64'), permissionsArray);
        
        // Verifies if the file was successfully shared with the user
        var result = await this.fileManager.accessControlContract.methods.getPermissionsOverFile(this.accountUserToShareFileWith, this.selectedFile.ipfsCID).call({from: this.fileManager.selectedUser.account});
        if (!result.success) {
