...
/*describe("AccessControl", function () {
    ...
    it("Should associate a user to a file giver certain permissions, if the user is not already associated", async function() {
        ...
        // Act
        const tx = await accessControl.shareFile(userAnaRita, file, "yourEncSymmetricKey3", ["download", "delete"]);
        await tx.wait();
    ...
    it("Should not associate a user to a file if the user is already associated to it", async function() {
        // Arrange        
        const { accessControl, userAnaRita, file } = await loadFixture(deployContractAndSetVariables);
        await accessControl.addUserHasFile(userAnaRita, file, "yourEncSymmetricKey3", ["download", "delete", "share"]);
    ...
    it("Should associate a file to the user and add the file, if the user is not already associated to it", async function() {
        // Arrange        
        const { accessControl, userAnaRita, file } = await loadFixture(deployContractAndSetVariables);

        // Act
        const tx = await accessControl.uploadFile(userAnaRita, file, "yourEncSymmetricKey3");
        await tx.wait();
    ...
    it("Shouldn't associate a file to the user and add the file, if the user is already associated to it", async function() {
        // Arrange        
        const { accessControl, userAnaRita, file } = await loadFixture(deployContractAndSetVariables);
        await accessControl.addUserHasFile(userAnaRita, file, "yourEncSymmetricKey3", ["download", "delete"]);

        // Act
        const tx = await accessControl.uploadFile(userAnaRita, file, "yourEncSymmetricKey1");
        await tx.wait();
